...
输入节点默认以 2 Hz 在终端打印左右手柄状态面板，内容包括当前按键、左右摇杆原始值、
...
`SL` 是持续按压式的运动使能键，也称为离合键。按下离合的瞬间，控制器记录：
...
3. 保持 Joy-Con 静止后按住 `SL`。
4. 从小幅度动作开始测试各个位置和姿态方向。
5. 如果方向不符合直觉，松开 `SL`，停止运行并修改 `teleop.yaml`。
